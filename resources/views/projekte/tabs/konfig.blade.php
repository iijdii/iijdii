{{-- Konfigurator-Tab = Produktpass: Informationen je Position; Änderungen
     laufen über das Konfigurator-Fenster (Modal, identisch zur Anfrage). --}}
@php
...
<div class="cols-2" style="grid-template-columns:minmax(0,1.6fr) minmax(280px,1fr);align-items:start">
    <div class="colstack">
        @forelse ($projekt->positionen as $position)
            <div class="card p0">
                <div class="card-h">
                    <span class="card-t">Position {{ $position->pos }} — {{ $position->produkt->label() }}</span>
                    <span class="fx ac gap8">
                        ...
                        <button class="btn btns" type="button" data-modal-target="posmodal-{{ $position->id }}">
                            <svg class="i"><use href="#ic-edit"/></svg>Konfigurator</button>
                ...
                <div class="card-b">
                    @if ($position->produkt->istDach())
                        ...
                        <div class="spec-row"><span class="spec-k">Lasten</span><span class="spec-v">{{ $k['snow'] }} · {{ $k['wind'] }}</span></div>
